...
public class Process {
        ...
        while (t < maxIteration && err > Problem.ERR_TOLERANCE) {
            ...
            for (int i = 0; i < swarmSize; i++) {
                ...
                newVel[2] = (w * p.getVelocity().getPos()[2])
                        + (r1 * c1) * (pBestLocation.get(i).getLoca()[2] - p.getLocation().getLoca()[2])
                        + (r2 * c2) * (gBestLocation.getLoca()[2] - p.getLocation().getLoca()[2]);
                newVel[3] = (w * p.getVelocity().getPos()[3])
                        + (r1 * c1) * (pBestLocation.get(i).getLoca()[3] - p.getLocation().getLoca()[3])
                        + (r2 * c2) * (gBestLocation.getLoca()[3] - p.getLocation().getLoca()[3]);
                ...
                newLoc[2] = p.getLocation().getLoca()[2] + newVel[2];
                newLoc[3] = p.getLocation().getLoca()[3] + newVel[3];
            ...
            System.out.println("     Best X: " + gBestLocation.getLoca()[0]);
            System.out.println("     Best Y: " + gBestLocation.getLoca()[1]);
    ...
    public void initSwarm() {
        ...
        for (int i = 0; i < swarmSize; i++) {
            ...
            loc[2] = Problem.LOC_U_LOW + ranGen.nextDouble() * (Problem.LOC_U_HIGH - Problem.LOC_U_LOW);
            loc[3] = Problem.LOC_W_LOW + ranGen.nextDouble() * (Problem.LOC_W_HIGH - Problem.LOC_W_LOW);
            ...
            vel[2] = Problem.VEL_LOW + ranGen.nextDouble() * (Problem.VEL_HIGH - Problem.VEL_LOW);
            vel[3] = Problem.VEL_LOW + ranGen.nextDouble() * (Problem.VEL_HIGH - Problem.VEL_LOW);
